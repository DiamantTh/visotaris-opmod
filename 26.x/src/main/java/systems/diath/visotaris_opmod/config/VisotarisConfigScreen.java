 ...
 * (Mojang trennt Zustands-Extraktion vom eigentlichen Render-Thread-Zeichnen),
 * drawString/drawCenteredString → text()/centeredText(), Button.render() →
 * Button.extractRenderState(), this.minecraft.setScreen() → this.minecraft.gui.setScreen()
 * (Screen-Öffnen/-Wechsel wanderte von Minecraft direkt in die neue Gui-Klasse).
 *
...
public final class VisotarisConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        addContent(Button.builder(
                ...
                b -> this.minecraft.gui.setScreen(new DiscordScreenshotSettingsScreen(this))
...
@Override
    public void extractRenderState(GuiGraphicsExtractor ctx, int mouseX, int mouseY, float delta) {
        // 1. Content-Buttons verstecken damit super.extractRenderState() sie nicht ungeschnitten zeichnet
        ...
        for (Button btn : contentButtons) {
            if (btn.visible) btn.extractRenderState(ctx, mouseX, mouseY, delta);
        }
        ...
        if (refreshStatus != null) {
            if (System.currentTimeMillis() - refreshStatusMs > 4_000L) {
                ...
                ctx.text(this.font, refreshStatus,
                        (this.width - tw) / 2,
    ...
    private void renderCategoryLabel(GuiGraphicsExtractor ctx, String text, int absY) {
        int cx   = this.width / 2;
        int tw   = this.font.width(text);
        int ty   = absY + (CAT_H - this.font.lineHeight) / 2;
        ctx.centeredText(this.font, Component.literal(text), cx, ty, 0xFFEEAA33);
                    ...
                    if (enabling) {
                        this.minecraft.gui.setScreen(new ConfirmScreen(
                                confirmed -> {
                                    ...
                                    this.minecraft.gui.setScreen(VisotarisConfigScreen.this);
                                },
    ...
    private void renderWebInterfaceFooterStatus(GuiGraphicsExtractor ctx, int footerTop, int mouseX, int mouseY) {
        var server = VisotarisModClient.getInstance().getWebServer();
        boolean running = server != null && server.isRunning();
     ...
     */
    private void renderGradientTitle(GuiGraphicsExtractor ctx) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            ctx.text(this.font, ch, x, y, color, true);
     ...
     */
    private void renderGradientSeparator(GuiGraphicsExtractor ctx) {
