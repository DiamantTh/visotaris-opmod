...
public final class PendingConfirmationService {
    ...
    public boolean queue(PendingAction.Type type, String command, String text) {
        ...
        pending.set(action);
        NoOpLogger.debug("PendingAction gespeichert: {} slot={}", type, slot);
        return true;
    }
    ...
    public void cancel(PendingAction.Type type) {
        ...
        if (action != null && action.getType() == type) {
            pending.set(null);
            NoOpLogger.debug("PendingAction abgebrochen: {}", type);
        }
    }
    ...
    public Intercepted tryIntercept(String rawCommand) {
        ...
        if (!queued) {
            NoOpLogger.debug("Bereits eine PendingAction ausstehend – neuer Intercept ignoriert.");
    ...
    private boolean validate(PendingAction action) {
        if (action.isExpired()) {
            NoOpLogger.debug("PendingAction abgelaufen.");
            return false;
        }
        MinecraftClient mc = MinecraftClient.getInstance();
        if (mc.player == null) return false;
        ...
        if (currentSlot != action.getSlot()) {
            NoOpLogger.debug("PendingAction ungültig: Slot gewechselt ({} → {}).",
        ...
        if (!currentFingerprint.equals(action.getItemFingerprint())) {
            NoOpLogger.debug("PendingAction ungültig: Item gewechselt.");
            return false;
        }
        return true;
    }
